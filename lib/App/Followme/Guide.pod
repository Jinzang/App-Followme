...
App::Followme::Readme - How to install, configure, and run followme
...
to the new method of each module as it ioaded, overriding the default values of
...
=item L<App::Followme::FormatPages>
...
=item L<App::Followme::ConvertPages>
...
Followme script is run on the directory or file passed as its argument. If
...
the type of the sigil. You can pass a reference to an array instead of a hash to
the subroutine this module generates. If you do, the template will use C<@data>
...
the L<App::Followme::Template> module.
...
where $configuration is a refererence to a hash containing the configuration
...
App::Followme::HandleSite to access its methods, which provide consistent
...
of the directory the module is being run on.
