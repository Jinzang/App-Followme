...
This module generates an index file from the most recently updated files in a
directory and its subdirectories. It also creates index files in each 
subdirectory listing each file. The variables described below in the variables
section are calculated for each file included in the indexes.

=item L<App::Followme::CreateRss>

This module creates an rss file from the metadata of the most recently updated 
files in a directory. It is a companion to App::Followme::CreateNews and should
