...
do {
    ...
    foreach my $filename (@filenames) {       
        my $input = App::Followme::read_page($filename);
        ok($input =~ /noarchive/, 'Followme changed template'); # tests 24,26
        ok($input =~ /Page/, "Followme kept contents"); # tests 25,27
    }
};
...
EOQ
    ...
    is($data->{loop}[0]{title}, 'Four', 'Index first page title'); # test 47
    is($data->{loop}[3]{title}, 'Two', 'Index last page title'); # test 48
