...
do {
    ...
    my $same = $hs->same_file('first.txt', 'first.txt');
    is($same, 1, 'Same file'); # test 1
    
    $same = $hs->same_file('first.txt', 'second.txt');
...
do {
    ...
    my $hs = App::Followme::HandleSite->new();
    is($hs->{base_directory}, $test_dir, 'Set HandleSite base directory');  # test 3

    my $excluded_files = $hs->glob_patterns($exclude_files);
    is_deeply($excluded_files, $excluded_files_ok, 'Glob patterns'); # test 4

    my $dir_ok = $test_dir;
    my $file_ok = 'index.html';
    my $filename = catfile($dir_ok, $file_ok);
    my ($dir, $file) = $hs->split_filename($filename);
...
EOQ
    ...
    my ($files, $folders) = $hs->visit($test_dir);
